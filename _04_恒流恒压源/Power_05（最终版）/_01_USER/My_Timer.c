...
unsigned char Watch_Flag = 0;  

/* 全局变量声明 ---------------------------------------------------------------*/
extern unsigned int IR2184_Timer;
...
{ 
   ...
   {  
        ...
        if(Watch_Flag == 1) { CLRWDT(); Watch_Flag = 0;}//喂狗    
        
        Timer_Cyc = (Timer_Cyc+1) % 100000000; //全局定时器   
        IR2184_Timer = (IR2184_Timer+1) % 10000; //全局定时器  
        if( Nix_Flash != 0 )  Nix_Flash --;
        
       /* 处理消息 --------------------------------*/ 
        if(Timer_Cyc%6 == 0) Manage_Message();   //60ms刷新一次消息           
        
       /* 显示结果 --------------------------------*/    
       if(Nix_Flash%30 == 0) Show_Result();   
