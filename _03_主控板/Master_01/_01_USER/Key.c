...
//    }
    ...
    /* 按键1 -----------------------------*/  
    if (gs_Local_Key_User.a_key_state[1-1] == KEY_short) g_RS485_Byte ++ ; //判断按键1是否短按
    if (gs_Local_Key_User.a_key_state[1-1] == KEY_hold)  g_RS485_Byte ++ ; //判断按键1是否长按
    
    /* 按键2 -----------------------------*/  
    if (gs_Local_Key_User.a_key_state[2-1] == KEY_short)   gs_Handle_Switch.Usart_OneDate = 1 ;//发送一组数据
    ...
    /* 按键5 ------------------------------*/  
    if (gs_Local_Key_User.a_key_state[4-1] == KEY_short)
    gs_Handle_Switch.Show_Send_Receive = ~gs_Handle_Switch.Show_Send_Receive ; //显示发送数据还是接收数据
