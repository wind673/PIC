...
void interrupt Sys_Cyc(void) 
{
   ...
   if(TMR0IF)
   {  
        SetTimer0(32); //重设溢出值 8 * 128us = 1024us = 1ms
        g_Timer_Cyc = (g_Timer_Cyc+1) % 4294967296; //全局定时器
       
        /* 指示灯闪烁 -----------------------------*/  
        if( g_Timer_Cyc%300 == 0 )  //运行指示灯闪烁
        {
        ...
        /* 发送查询指令 ----------------------------*/
        if( g_Timer_Cyc % INQUIRY_INTERVAL == 0 )
        gs_Switch.Inquiry = 1;                           
    ...
    {
        _temp = RCREG1;   
        if( gs_Switch.Receive == 0 ) //接收标志已复位
        {
            /* 起始码 --------------------------------*/ 
