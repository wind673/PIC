...
void Key_ManageMessage(void)
{
    ...
    {
        if(g_RS485_Byte >=1 && g_RS485_Byte <=3)
        gs_Handle_Switch.Relay[g_RS485_Byte - 1] = 1;  //继电器开         
    ...
    {
        if(g_RS485_Byte >=1 && g_RS485_Byte <=3)
        gs_Handle_Switch.Relay[g_RS485_Byte - 1] = 0;//继电器关
        Key_ResetFSM();  //读完组合按键，则其他按键皆无效
    }
    ...
    /* 按键2（菜单）-----------------------------*/  
    if (gs_Local_Key_User.a_key_state[2-1] == KEY_short)   gs_Handle_Switch.Show_Send_Receive  = ~gs_Handle_Switch.Show_Send_Receive; //显示发送数据还是接收数据
    ...
    gs_Handle_Switch.Usart_OneDate = 1;//发送一组数据                                                                                  
