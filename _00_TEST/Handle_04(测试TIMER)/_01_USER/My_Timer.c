...
{
    static unsigned char _overturn = 0;
   /*---------------- TIMER0 ----------------*/ 
   if(TMR0IF)
   { 
       if( _1Khz_Switch)
       {
        if(_overturn) LED1_ON; else LED1_OFF;
         _overturn =~_overturn;
       }
        SetTimer0(450); //重设溢出值 500us
        Timer_Cyc = (Timer_Cyc+1) % 10000000; //全局定时器 单位： per 10ms
