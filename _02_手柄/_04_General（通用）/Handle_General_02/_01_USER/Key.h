...
typedef struct Key_Struct
{
 ...
 volatile uint key_count[KEY_NUMBER] ; //RC2~5计数
 volatile uchar key_state[KEY_NUMBER] ; //按键状态
}__Def_Key_FSM; 
...
extern __Def_Key_FSM  Local_Key_User;

/* 全局函数声明 ---------------------------------------------------------------*/  
void Key_Init(void);
void Key_FSM_Scan(void);
void Key_FSM_Reset(void);//状态机复位
void Key_FSM_Read(void); //读取状态机的状态
void Key_Message_Manage(void); //按键消息处理
