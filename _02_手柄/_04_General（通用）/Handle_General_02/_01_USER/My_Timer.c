...
void interrupt Sys_Cyc(void) 
{
    ...
    static char _RS485_Receive_Byte; //RS485接收位
    static char _flag = 0; 
    unsigned char _temp;
   ...
   {  
        ...
        /* 1KHZ脉冲输出 --------------------------------*/ 
        if( Handle_Switch._1Khz ) //1KHZ脉冲输出
         Pulse_1Khz();
    ...
    {
        _temp = RCREG2; 
        if( RS485_Receive_Flag == 0 ) //接收标志已复位
        {
            if( _temp == RS485_START ) //起始码
            {
                 _RS485_Receive_Byte = RS485DATA_LENTH ;
                 _flag = 0;
             }   
            ...
            {
                 _RS485_Receive_Byte -- ; 
                 RS485_ReceiveBuff[ _RS485_Receive_Byte ] = _temp; //接收数据   
             }
                
             if( _RS485_Receive_Byte == 0 )  
             {
                 RS485_Receive_Flag = 1 ;  //接收1组数据完毕
                 _flag = 1;
             }
        }
    }
