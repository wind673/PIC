...
void Key_FSM_Scan(void)
{
    ...
    if( Local_Key_FSM.key_state & 0x10 ) Local_Key_FSM.key[4]++;//判断按键3的状态
...
unsigned char Key_FSM_Read( unsigned char key_num)
{
    ...
    if( key_num != KEY_group ) //单独按键数据
    {
        if( Local_Key_FSM.key_state == 0x00 )  //按键空闲状态下
        {
            ...
            Local_Key_FSM.key_FSM[key_num-1] = KEY_default; //重置该按键的状态机
            return temp;
        }
        //按键繁忙状态下返回忙碌状态(被按下)
        else return KEY_busy;
    }
    else if ( key_num == KEY_group ) //组合按键数据
    {
        if( Local_Key_FSM.key_state == 0x00 )  //按键空闲状态下
        {
